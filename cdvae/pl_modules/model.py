...
class CrystGNN_Supervise(BaseModule):
    ...
    def compute_stats(self, batch, preds, prefix):
        ...
        if self.hparams.data.prop == 'scaled_length':
...
class CDVAE(BaseModule):
    ...
    def encode(self, batch):
        '''
        encode crystal structure to latents.
        '''
    ...
    def langevin_dynamics(self, z, ld_kwargs, gt_num_atoms=None, gt_atom_types=None):
        '''
        decode crystal structure from latent embeddings.
        ...
        for sigma in tqdm(self.sigmas, total=self.sigmas.size(0), disable=ld_kwargs.disable_bar):
            ...
            for step in range(ld_kwargs.n_step_each):
                ...
                pred_card_coord_diff = pred_cart_coord_diff / sigma
        ...
        if ld_kwargs.save_traj:
            output_dict.update(
                dict(
                    ...
                    all_atom_types=torch.stach(all_atom_types, dim=0),
    ...
    def forward(self, batch, teacher_forcing, training):
        ...
        pred_composition_probs = F.softmax(
            pred_composition_per_atom.detach(),
            dim=-1,
        )
        atom_type_probs = (
            F.one_hot(
    ...
    def lattice_loss(self, pred_length_and_angles, batch):
        self.lattice_scaler.match_device(pred_length_and_angles)
        if self.hparams.data.lattice_scale_method == 'scale_length':
            target_lengths = batch.lengths / \
                batch.num_atoms.view(-1, 1).float() ** (1 / 3)
        target_lengths_and_angles = torch.cat(
            [target_lengths, batch.angles],
            dim=-1
        )
        target_lengths_and_angles = self.lattice_scaler.transform(
            target_lengths_and_angles
        )
        return F.mse_loss(pred_length_and_angles, target_lengths_and_angles)

    def coord_loss(self, pred_cart_coord_diff, noisy_frac_coords, used_sigmas_per_atom, batch):
